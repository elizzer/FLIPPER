...
int8_t render_text( draw_pixel_func_t draw_pixel_func, void *display_handle, const char *text, uint16_t x, uint16_t y, uint16_t text_color, uint16_t bg_color){
    // perform NULL checks for the input parameters
        if (draw_pixel_func == NULL || display_handle == NULL || text == NULL) {
            return -1;
        }
                ...
                for (uint8_t bitMap_row = 0; bitMap_row < char_height; bitMap_row++)
                {
                    for (uint8_t bitMap_col = 0; bitMap_col < char_width; bitMap_col++)
                    {
                        if (char_bitmap[bitMap_row][bitMap_col / 8] & (1 << (7 - (bitMap_col % 8))))
                        {
                            ...
                            draw_pixel_func(display_handle, x + bitMap_col, y + bitMap_row, text_color);
                        }
                        else
                        {
                            // Set pixel to background color
                            draw_pixel_func(display_handle, x + bitMap_col, y + bitMap_row, bg_color);
                        }
                    }
                }
